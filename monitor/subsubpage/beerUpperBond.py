...
class BeerUpperBondPage(tk.Frame):
    def __init__(self, master, controller):
        ...
        # Title
        label_title = tk.Label(self, text="啤酒出酒上限設定", font=("Helvetica", 20))
        ...
        self.slider = tk.Scale(center_frame, from_=800, to=0, orient=tk.VERTICAL, 
        ...
        # 警示文字
        self.alert_label = tk.Label(center_frame, text=f"啤酒出酒上限為{self.slider.get()}毫升(ml)", font=("Helvetica", 18), fg="red")
        self.alert_label.grid(row=0, column=1, padx=20)  # 使用grid佈局，且位於滑桿右側

        # 從CSV讀取值
        initial_value = self.read_value_from_csv("BeerUpperBond")
        self.slider.set(initial_value if initial_value is not None else 0)


    def update_value(self, value):
        self.alert_label.config(text=f"啤酒出酒上限為{value}毫升(ml)")
        self.save_value_to_csv("BeerUpperBond",value) # 存至 CSV 中
